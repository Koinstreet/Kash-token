...
/**
 * @title KASH token initial distribution
 *
 * @dev Distribute purchasers, airdrop, reserve, and founder tokens
 */
contract KashDistribution is Ownable {
  using SafeMath for uint256;

  KashToken public KASH;
  ...
  // Keeps track of whether or not a 250 KASH airdrop has been made to a particular address
  ...
  event LogKashClaimed(address indexed _recipient, uint8 indexed _fromSupply, uint256 _amountClaimed, uint256 _totalAllocated, uint256 _grandTotalClaimed);

  /**
    * @dev Constructor function - Set the poly token address
    * @param _startTime The time when KashDistribution goes live
    */
  function KashDistribution(uint256 _startTime) public {
    ...
    KASH = new KashToken(this);
  }

  /**
    * @dev Allow the owner of the contract to assign a new allocation
    * @param _recipient The recipient of the allocation
    * @param _totalAllocated The total amount of KASH available to the receipient (after vesting)
    * @param _supply The KASH supply the allocation will be taken from
    */
  ...
  function airdropTokens(address[] _recipient) public onlyOwnerOrAdmin {
    ...
    for(uint256 i = 0; i< _recipient.length; i++)
    {
        if (!airdrops[_recipient[i]]) {
          ...
          require(KASH.transfer(_recipient[i], 250 * decimalFactor));
  ...
  function transferTokens (address _recipient) public {
    ...
    require(KASH.transfer(_recipient, tokensToTransfer));
    grandTotalClaimed = grandTotalClaimed.add(tokensToTransfer);
    LogKashClaimed(_recipient, allocations[_recipient].AllocationSupply, tokensToTransfer, newAmountClaimed, grandTotalClaimed);
  }

  // Returns the amount of KASH allocated
